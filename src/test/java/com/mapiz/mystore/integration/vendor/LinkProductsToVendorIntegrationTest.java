...
class LinkProductsToVendorIntegrationTest extends BaseIntegrationTest {
    ...
    void testLinkProducts() throws Exception {
        ...
        when(productVendorRepository.saveAll(any())).thenReturn(List.of(
                ProductVendorEntity.builder().id(1).vendor(savedVendor).product(savedProducts.get(0)).price(10.0).build(),
                ProductVendorEntity.builder().id(2).vendor(savedVendor).product(savedProducts.get(1)).price(20.0).build()
        ));
        ...
        var expectedProductVendors = List.of(
                ProductVendorResponse.builder().id(1)
                        .product(
                                ProductResponse.builder().id(1).name("Product 1").description("Description 1").build()
                        )
                        //.vendor(vendorResponse)
                        .price(10.0).build(),
                ProductVendorResponse.builder().id(2)
                        .product(
                                ProductResponse.builder().id(2).name("Product 2").description("Description 2").build()
                        )
                        //.vendor(vendorResponse)
                        .price(20.0).build()
        );

        mockMvc.perform(MockMvcRequestBuilders.post(EndpointConstant.VENDORS_BASE_PATH + "/" + vendorId + "/link-products")

...
class DeleteProductIntegrationTest extends BaseIntegrationTest {
    ...
    void testDeleteExistingProduct() throws Exception {
        // Arrange
        var idToDelete = 1;
        var productToDelete = ProductEntity.builder().id(1).name("Product 1").description("product one").build();
        when(productRepository.findById(idToDelete)).thenReturn(Optional.ofNullable(productToDelete));

        // Act & Assert
        mockMvc.perform(MockMvcRequestBuilders.delete(EndpointConstant.PRODUCTS_BASE_PATH + "/" + idToDelete))
    ...
    void testDeleteNonExistentProduct() throws Exception {
        ...
        var expectedApiError = new ApiError("resource_not_found", "Product not found", HttpStatus.NOT_FOUND.value());

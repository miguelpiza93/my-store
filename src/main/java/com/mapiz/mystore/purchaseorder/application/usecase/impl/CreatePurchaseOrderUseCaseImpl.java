...
//        var productsToOrder = productRepository.findAllById(
//                command.getPurchaseOrderLines().stream().map(PurchaseOrderLineRequest::getProductId)
...
//        );
        ...
        ).stream().map(ProductVendorMapper.INSTANCE::entityToModel).toList();
